...
# Script qiime_fq.sh: para dados trimados a priori
...
# Script versão 0.03, terminado dia 07/12/2016 ás 16:34h. Parte deste shell
...
# Caminho absoluto para o banco de dados do green verssão 119 (o mais atual até
#    27/04/2016, https://www.arb-green.de/documentation/release-119/)
    green="/usr/local/bioinfo/miniconda2/envs/qiime/lib/python2.7/site-packages/qiime_test_data/align_seqs/core_set_aligned.fasta.imputed"
...
# Caminho absoluto para os scripts do BMP
    bmp_map="bmp-map2qiime.py"

# Caminho para os arquivos 97_otus.fasta e 97_otu_taxonomy.txt usado por Picrust
    picru_97_fa="/usr/local/bioinfo/miniconda2/envs/qiime/lib/python2.7/site-packages/qiime_default_reference/gg_13_8_otus/rep_set/97_otus.fasta"
    picru_97_tx="/usr/local/bioinfo/miniconda2/envs/qiime/lib/python2.7/site-packages/qiime_default_reference/gg_13_8_otus/rep_set/97_otu_taxonomy.txt"
...
#========================================================================
               for fq in `ls ${base_dir}/*.fastq` ; do
                # fqname = string que contém o nome dos arquivos fastq
               fqname=`basename ${fq} .fastq`
               mv ${base_dir}/${fq} ${base_dir}/${trim}


# Listando os arquivos fastq recém movidos ./trim_data/fastq_fna_dir e
#    capturando a string do nome que será usada pelo pipeline do QIIME
               for fastq in `ls ${trim}/*.fastq` ; do
               fqname=`basename ${fastq} .fastq`

       done;

       echo "          Convertendo o arquivo ${fqname}.fastq para ${fqname}.fna e ${fqname}.qual (convert_fastaqual_fastq.py) ... "
       convert_fastaqual_fastq.py -c fastq_to_fastaqual -f ${fastq} -o ${trim} 1> ${logs}/${fqname}_convert_fq_fa_out.txt 2> ${logs}/convert_fq_fa_err.txt

done;

        echo " Removendo os arquivos ${fqname}.qual de todas as amostras ... "
        rm -f ${trim}/*.qual
...
#========================================================================
        ...
        align_seqs.py -m muscle -i ${conc}/otus.fa -o ${rep}/rep_set_align -t ${green} -p 0.97 1> ${logs}/align_seqs_out.txt 2> ${logs}/align_seqs_err.txt
