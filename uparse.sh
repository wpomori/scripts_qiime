...
echo "      Derreplicação ... "
# Dereplicação
/home/softwares/usearch/usearch -derep_fulllength ${conc}/all.fna -fastaout ${conc}/all_derep.fa -sizeout

echo "      Removendo singletons ... "
# Removendo singletons antes de criar as OTUs representativas
/home/softwares/usearch/usearch -sortbysize ${conc}/all_derep.fa -fastaout ${conc}/all_sorted_min2.fa -minsize 2

echo "      Clusterização de novo ... "
# Clusterização de novo
/home/softwares/usearch/usearch -cluster_otus ${conc}/all_sorted_min2.fa -otus ${conc}/all_otu_reps_init.fa -uc ${conc}/all_otu_reps_init.up -relabel OTU_ -sizein -sizeout

echo "      Removendo quimeras usando banco de dados referência ... "
# Remoção de quimera baseado em referência
/home/softwares/usearch/usearch -uchime_ref ${conc}/all_otu_reps_init.fa -db ${gold} -strand plus -nonchimeras ${conc}/all_otu_reps.fa


fasta_formatter -i ${conc}/all_otu_reps.fa -o ${conc}/all_otu_reps_formated.fa

# Download scripts BMP: https://github.com/vpylro/BMP
perl /home/softwares/BMP-master/bmp-otuName.pl -i ${conc}/all_otu_reps_formated.fa -o ${conc}/otus.fa 


echo "      Mapeando as reads contra as OTUs representativas ... "
# Mapear as reads contra as OTUs representativas
/home/softwares/usearch/usearch -usearch_global ${conc}/all.fna -db ${conc}/otus.fa -strand plus -id 0.97 -uc ${conc}/all_otu_map.uc
